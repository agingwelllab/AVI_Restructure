aclean <- function(araw_df){
  ...
  a_actual_long <- select(a_actual_long, -'A/I') #remove column identifying if variable is for actual or ideal affect
...
aoctant <- function(atotal_clean){
  ...
  atotal_mean <- aggregate(cbind(actual,ideal) ~ record_id + octant, data = atotal_clean, FUN = mean)
